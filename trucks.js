var trucks = (function() {
  ...
  function getFoodTypes() {
    ...
    for (var i=0; i<foodTrucks.length; i++) {
      for (var j=0; j<foodTrucks[i].type.length; j++) {
        temp.push(foodTrucks[i].type[j]);
      }
    }

  //return unique food types only
  ...
  function filterTrucksByFoodType(foodType) {
    return foodTrucks.filter(function(item) {
      for (var i=0; i<item.type.length; i++) {
        if(item.type[i].toUpperCase() == foodType.toUpperCase()) {
          return true;
        }
      }
    });
  ...
  //add new truck, input is a truck object
  function addTruck(newTruck) {
    foodTrucks.push(newTruck);
  }

  //remove a truck from current truck list based the truck's name
  function removeTruck(truckName) {
    for (var i = 0; i < foodTrucks.length; i++) {
      if (foodTrucks[i].name.toUpperCase() == truckName.toUpperCase()) {
