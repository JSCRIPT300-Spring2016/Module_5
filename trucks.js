...
// return the entire food truck array
function getTrucks() {
	return foodTrucks;
}

// find a truck named 'name' in the foodTrucks array
function getTruck(name) {
	return _.find(foodTrucks,
		function(t) {
			return t.name.toLowerCase() === name.toLowerCase();
		});
}

// return a list of unique food types
function getFoodTypes() {

	var foodTypes = [];

	for (var i = 0; i < foodTrucks.length; ++i) {
		for (var j = 0; j < foodTrucks[i].type.length; ++j) {
			// if ft is not in our list then add it
			var foundFoodType =
				_.find(foodTypes,
					function(f) {
						return f.toLowerCase() === foodTrucks[i].type[j].toLowerCase();
					});

			if (foundFoodType === undefined) {
				foodTypes.push(_.startCase(foodTrucks[i].type[j]));
			}
		}
	}
...
function filterByDay(day) {
	return _.filter(foodTrucks,
		function(t) {
			return undefined !== _.find(t.schedule,
				function(d) {
...
function filterByFoodType(foodType) {
	return _.filter(foodTrucks,
		function(t) {
			return undefined !== _.find(t.type,
				function(f) {
					return f.toLowerCase() === foodType.toLowerCase();
				});
		});
}

function addTruck(newTruck) {
	if (newTruck && newTruck.hasOwnProperty("name")) {
		foodTrucks.push(newTruck);
	}
}

function removeTruck(name) {
	var foundTruck = _.find(foodTrucks,
		function(t) {
			return t.name.toLowerCase() === name.toLowerCase();
		});

	if (foundTruck) {
		foodTrucks.splice(foodTrucks.indexOf(foundTruck), 1);
	}
